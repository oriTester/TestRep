...
public abstract class RosActivity extends AppCompatActivity {
    ...
    public void execute(int requestCode, int resultCode, Intent data) {
      if (resultCode == RESULT_OK) {
        if (requestCode == MASTER_CHOOSER_REQUEST_CODE) {
          ...
          new initTask(RosActivity.this).execute();
        } else {
  ...
  private static class initTask extends AsyncTask<Void, Void, Void>{
    ...
    private WeakReference<RosActivity> activityReference;

    initTask(RosActivity context) {
      super();
    ...
    protected Void doInBackground(Void... voids) {
      RosActivity activity = activityReference.get();
      activity.init(activity.nodeMainExecutorService);
      return null;
    }
  }
}
